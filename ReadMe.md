...
- `.bcp` 文件中的 `【用例标题】` 用作 `casename` 匹配来源。
...
- 如果 section 中额外提供 `casename = 某个标题`，则会优先使用该值匹配 `.nb` 中的记录。
- 如果未提供 `casename`，程序会按 section 顺序自动使用 `.bcp` 文件中 `【】` 内的标题作为 `casename`。

### 6.2 .bcp 文件

规则如下：

- `.bcp` 文件中的 `【用例标题】` 用于提取 `casename`。
- 程序会扫描当前功能目录下 `testdata` 中所有 `.bcp` 文件，按文件名排序后按出现顺序提取 `casename`。
...
3. 每条记录中的字段使用 `tab` 分隔。
4. 字段序号从 `1` 开始计数。
5. 程序默认将每条记录的第 `1` 列视为 `casename`，并据此匹配测试用例。

校验流程：

1. 从 `testcase.ini` 和 `.bcp` 中得到期望用例与 `casename`。
2. 通过数据库查询得到每个 `guid` 对应的输出序号。
3. 读取 `.nb` 文件，按 `casename` 匹配目标记录。
...
2. 扫描 `.bcp` 文件中的 `【用例标题】`，生成或补全 `casename`。
...
1. `.nb` 文件的第 1 列为 `casename`。
2. `.mylog` 文件名不做固定格式限制，只要位于 `std_log` 目录下且扩展名为 `.mylog` 即会被扫描。
3. 等待 `.nb` 文件时，只认启动之后新生成或被修改的 `.nb` 文件。
4. `testcase.ini` 未显式写 `casename` 时，按 section 顺序与 `.bcp` 中出现的 `【用例标题】` 顺序一一对应。
